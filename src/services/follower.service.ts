...
class FollowService {
  ...
  create = async (
    ...
    try {
      const existingFollow = await followRepository.getOne({
        follower,
        followed,
      });
      ...
      if (existingFollow) {
        ...
        return this.Response.sendSuccessResponse("Unfollowed successfully", {
          follow: false,
        });
      }
      ...
      return this.Response.sendSuccessResponse("User followed successfully", {
        follow: true,
      });
    } catch (error) {
  ...
  getOne = async (follower: string, followed: string): Promise<ApiResponse> => {
    try {
      ...
      if (!follow) {
        return this.Response.sendSuccessResponse("Not Followed", {
          isFollowing: false,
        });
      }
      return this.Response.sendSuccessResponse("Not Followed", {
