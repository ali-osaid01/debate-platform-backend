...
class CategoryService {
  ...
  create = async (payload: ICategory): Promise<ApiResponse> => {
    const isCategoryExist = await categoryRepository.getOne({
      title: payload.title,
    });
    if (isCategoryExist)
      return this.Response.sendResponse(409, "Category already Exist");
  ...
  index = async (page: number, limit: number, filter: PipelineStage[]) => {
    const result = await categoryRepository.getAllAggregated({
      query: filter,
      page,
      limit,
    });
    if (result.data.length == 0)
      return this.Response.sendResponse(404, "Category Not Found");
    return this.Response.sendSuccessResponse(
  ...
  show = async (filter: Partial<ICategory>) => {
